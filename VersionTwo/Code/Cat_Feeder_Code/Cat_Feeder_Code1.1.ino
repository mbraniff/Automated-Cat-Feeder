 ...
 * 
 * Change Log:
 * Fixed bug with calculating next drop time.
 * Got rid of the idea of a 2D array holding the drop time information and decided to go with an array of structs
 * Added type definition of struct feedTime that holds 4 chars (8 bit values)
 * Added constructor function for feedTime structs 
 * Added function to increases button while not editing time. It can now switch on and off backlight to conserve power.
 * Changed all global int values to char (8 bit values) so they take up less memory and to prepare to store values in EEPROM.
 * Added Boolean for backlight on or off
 */
...
const int MILLISPEED = 250; //This is for testing, this should be at 1000 for normal time tracking
...
void calcFeedTime(char feedTime[]){
  ...
  if(feedTimes[0].ind == 'P'){   //If our indicator value is P
    if(feedTimes[0].h == 12){ //If its 12PM drop hour is 12
      dropHours = 12;
    }else{                    //Else add 12 to hour reading
      dropHours = feedTimes[0].h + 12;
    }
  }else{ //If its AM
    if(feedTimes[0].h == 12){//If its 12AM drop hour is 0
      dropHours = 0;
    }else{                    //Else drop hour is just the reading
      dropHours = feedTimes[0].h;
    }
  }
  ...
  if(hDiff > 2){
    ...
    if(hDiff == 2){
      int mDiff = (feedTimes[0].m + 60)-m; //If hour difference is 2 see if min diff is > 60
      if(mDiff > 60){
      ...
      int mDiff = (feedTimes[0].m + 60)-m;
     sprintf(feedTime,"%02dM",mDiff);//Else print 
     return; 
    }else{
      sprintf(feedTime,"%02dH",hDiff);//Else print
      return; 
    }
  }else{ //If Hour Difference >= 0 and <= 2
    int mDiff; // Minute difference
    if(hDiff == 1){ //If hour Difference is 1
      mDiff = (feedTimes[0].m+60)-m; //Add an hour to feed time and subract current minutes
      if(mDiff > 60){ //If min Difference is over an hour
        ...
        int sDiff = (feedTimes[0].s + 60) - s;
        if(sDiff > 60){
          sprintf(feedTime,">1M");
          return;
        }else{
          sprintf(feedTime,"%02dS",sDiff);
          return; //Recently fixed bug
        }
      }
    }else if(hDiff == 2){
      mDiff = (feedTimes[0].m + 60)-m; //If hour difference is 2 see if min diff is > 60
      if(mDiff > 60){
        sprintf(feedTime,"%2dH",hDiff); //If it is then print hDiff (2)
        return;
      }else{
      ...
      mDiff = feedTimes[0].m-m; // Set minute difference
      if(mDiff > 1){
        ...
        if(mDiff > 0){ 
          sDiff = (feedTimes[0].s+60) - s; //If min diff = 1 add 60 to feed time seconds
          if(sDiff > 60){
          ...
          sDiff = (feedTimes[0].s - s); //Find second difference
          if(sDiff > 0){
